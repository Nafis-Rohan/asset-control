            ...
            <div class="rounded-lg bg-white p-6 shadow-sm">
                <form method="POST" action="{{ route('assets.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="category" name="category" type="text" class="mt-1 block w-full" :value="old('category')" placeholder="Laptop, Monitor, License..." required />
                    ...
                    <div>
                        ...
                        <select id="user_id" name="user_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach ($employees as $employee)
                                <option value="{{ $employee->id }}" @selected(old('user_id') == $employee->id)>{{ $employee->name }} ({{ $employee->department }})</option>
                            @endforeach
                        </select>
